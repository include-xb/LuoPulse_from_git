...
func _ready():
	...
	for item in GlobalScene.selectable_list + GlobalScene.individual_msc_list:
		print(GlobalScene.selectable_msc_path + item)
		
		var instanced_demo_msc : MarginContainer = packed_demo_msc.instantiate()
		scroll_body.add_child(instanced_demo_msc)
		instanced_demo_msc.set_demo_msc(item, true)
